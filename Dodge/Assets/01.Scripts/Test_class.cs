...
public class Test_class : MonoBehaviour
{
    ...
    void Start()
    {
        junhee = new Human();       //Human타입의 객체를 만들어 줬다.(인스턴스화)
        junhee.age = 25;            //사람의 나이를 정해줌
        junhee.name = "준희";      //사람의 이름을 정해줌
        ...
        Debug.Log("나이 : " + junhee.age + "나이 :" + junhee_1.age);        //클래스가 참조타입이라 junhee_1의 age(나이)를 바꿔도 junhee변수 안에 있는 age라는 값이 바꼈다.
        Debug.Log("나이 : " + junhee_1.age);
...
public class Test_human2 : Human
{
    ...
    public class Test_Human2 : Human
    {
        void Test_method()
        {
            test_protected = "테스트용 문자열";   //protected로 선언이 돼어있어 외부에선 접근이 불가능하지만 상속받은 클래스에선 접근이 가능하다
        }
    }
}
